// Italiano e inglese per la vista cliente.
// L'italiano e' la lingua sorgente: se una traduzione inglese manca, si mostra
// l'italiano invece di lasciare un buco. Le etichette dei 14 allergeni UE non
// passano di qui: arrivano gia' tradotte dal database.

export const LOCALES = ['it', 'en'];

const DICTIONARY = {
  it: {
    'app.tagline': 'Croccante fuori, filante dentro',
    'app.headline': 'Il tuo incontro con la pizza.',
    'app.subtitle': 'Sfornata ora. La ritiri calda a Milano, Piola.',
    'hero.weekly': 'Pizza della settimana',
    'hero.today': 'Oggi esce dal forno',
    'hero.cta': 'Voglio questa',
    'hero.social': 'La più gettonata della settimana',
    'status.open': 'Ordini aperti',
    'status.closed': 'Ordini al momento chiusi',
    'status.wait': 'Attesa indicativa',
    'status.minutes': 'minuti',
    'tabs.pizzas': 'Pizze',
    'tabs.drinks': 'Bibite',
    'tabs.cart': 'Carrello',
    'menu.search': 'Cerca nel menu…',
    'menu.searchEmpty': 'Nessun piatto trovato con questo nome.',
    'product.customize': 'Aggiungi',
    'confirm.title': 'Controlla e conferma',
    'confirm.sub': 'Una volta inviato, la pizza va in forno.',
    'confirm.when': 'Pronto tra circa',
    'booking.question': 'Quando la vuoi?',
    'booking.now': 'Appena pronta',
    'booking.later': 'Prenotala per',
    'booking.today': 'oggi',
    'booking.lunch': 'Pranzo',
    'booking.dinner': 'Sera',
    'booking.at': 'le',
    'booking.confirmed': 'Prenotata per',
    'booking.off': 'Le prenotazioni non sono attive al momento.',
    'booking.pick': 'Scegli un orario valido per la prenotazione.',
    'booking.full': 'Quell’orario si è riempito proprio ora: scegline un altro.',
    'booking.closedDay': 'Quel giorno la pizzeria è chiusa: scegli un altro orario.',
    'confirm.who': 'A nome di',
    'confirm.phone': 'Ti avvisiamo al',
    'confirm.add': 'Vuoi aggiungere?',
    'confirm.back': 'Torna al carrello',
    'confirm.send': 'Conferma l\u2019ordine',
    'confirm.pay': 'Paga',
    'name.missing': 'Scrivi il tuo nome: serve per chiamarti quando la pizza esce.',
    'name.fake': 'Questo non sembra un nome: scrivi quello vero.',
    'name.confirm': 'e davvero il tuo nome?',
    'name.confirmYes': 'Si, mi chiamo cosi',
    'name.confirmNo': 'Lo correggo',
    'wait.title': 'Sala d\u2019attesa',
    'wait.left': 'ancora',
    'wait.minutes': 'minuti',
    'wait.now': 'da un momento all\u2019altro',
    'wait.order': 'Il tuo ordine',
    'wait.live': 'Questa pagina si aggiorna da sola.',
    'confirm.demo': 'Pagamento dimostrativo: nessun addebito viene eseguito.',
    'product.closed': 'Ordini chiusi',
    'product.drink': 'Fresca e dissetante',
    'cart.title': 'Il tuo ordine',
    'cart.close': 'Chiudi',
    'cart.empty': 'Il carrello è vuoto.',
    'cart.total': 'Totale',
    'cart.name': 'Nome',
    'cart.phone': 'Telefono',
    'cart.email': 'Email (facoltativa)',
    'cart.without': 'Senza',
    'cart.noNote': 'Nessuna nota',
    'cart.remove': 'Rimuovi',
    'cart.payment': 'Pagamento dimostrativo',
    'cart.confirm': 'Conferma ordine demo',
    'recap.title': 'Ordine confermato',
    'recap.code': 'Il tuo numero',
    'recap.ready': 'Pronto tra circa',
    'recap.payment': 'Pagamento',
    'recap.total': 'Totale',
    'recap.allergens': 'Allergeni',
    'recap.note': 'Nota',
    'recap.contact': 'Se qualcosa non va, chiama la pizzeria.',
    'recap.sendSms': 'Invia il riepilogo via messaggio',
    'recap.sendEmail': 'Invia il riepilogo via email',
    'recap.sent': 'Invio dimostrativo: nella versione finale il riepilogo arriva davvero.',
    'recap.newOrder': 'Fai un altro ordine',
    'recap.demo': 'Ordine dimostrativo: nessun pagamento reale è stato eseguito.',
    'payment.cash': 'Paga in cassa',
    'payment.apple_pay': 'Apple Pay · demo',
    'payment.google_pay': 'Google Pay · demo',
    'custom.title': 'Personalizza la tua pizza',
    'custom.included': 'Ingredienti inclusi',
    'custom.additions': 'Aggiunte',
    'custom.removed': 'TOLTO',
    'custom.kept': 'INCLUSO',
    'custom.allergens': 'Allergeni',
    'custom.none': 'nessuno dichiarato',
    'custom.note': 'Note per questa pizza',
    'custom.notePlaceholder': 'Es. allergia alle noci, celiaco, ben cotta…',
    'custom.add': 'Aggiungi al carrello',
    'custom.from': 'da',
    'captcha.missing': 'Conferma di non essere un robot (la casella qui sopra), poi invia.',
    'custom.base': 'prezzo base',
    'allergens.warning': 'In caso di allergie o intolleranze scrivilo nelle note e contatta il locale. Può verificarsi contaminazione crociata.'
  },
  en: {
    'app.tagline': 'Crispy outside, melty inside',
    'app.headline': 'Your meeting with pizza.',
    'app.subtitle': 'Fresh out of the oven. Pick it up hot in Milan, Piola.',
    'hero.weekly': 'Pizza of the week',
    'hero.today': 'Fresh out of the oven today',
    'hero.cta': 'I want this one',
    'hero.social': "This week's most wanted",
    'status.open': 'Orders open',
    'status.closed': 'Orders currently closed',
    'status.wait': 'Estimated wait',
    'status.minutes': 'minutes',
    'tabs.pizzas': 'Pizzas',
    'tabs.drinks': 'Drinks',
    'tabs.cart': 'Cart',
    'menu.search': 'Search the menu…',
    'menu.searchEmpty': 'No dish found with that name.',
    'product.customize': 'Add',
    'confirm.title': 'Check and confirm',
    'confirm.sub': 'Once sent, the pizza goes in the oven.',
    'confirm.when': 'Ready in about',
    'booking.question': 'When do you want it?',
    'booking.now': 'As soon as it’s ready',
    'booking.later': 'Book it for',
    'booking.today': 'today',
    'booking.lunch': 'Lunch',
    'booking.dinner': 'Dinner',
    'booking.at': '',
    'booking.confirmed': 'Booked for',
    'booking.off': 'Bookings are not open right now.',
    'booking.pick': 'Pick a valid time for your booking.',
    'booking.full': 'That time just filled up: pick another one.',
    'booking.closedDay': 'The pizzeria is closed that day: pick another time.',
    'confirm.who': 'Under the name',
    'confirm.phone': 'We will text you at',
    'confirm.add': 'Anything else?',
    'confirm.back': 'Back to the cart',
    'confirm.send': 'Confirm the order',
    'confirm.pay': 'Pay',
    'name.missing': 'Write your name: we call it out when the pizza is ready.',
    'name.fake': 'That does not look like a name: please write your real one.',
    'name.confirm': 'is that really your name?',
    'name.confirmYes': 'Yes, that is my name',
    'name.confirmNo': 'Let me fix it',
    'wait.title': 'Waiting room',
    'wait.left': 'still',
    'wait.minutes': 'minutes',
    'wait.now': 'any moment now',
    'wait.order': 'Your order',
    'wait.live': 'This page updates on its own.',
    'confirm.demo': 'Demo payment: nothing is actually charged.',
    'product.closed': 'Orders closed',
    'product.drink': 'Cold and refreshing',
    'cart.title': 'Your order',
    'cart.close': 'Close',
    'cart.empty': 'Your cart is empty.',
    'cart.total': 'Total',
    'cart.name': 'Name',
    'cart.phone': 'Phone',
    'cart.email': 'Email (optional)',
    'cart.without': 'Without',
    'cart.noNote': 'No note',
    'cart.remove': 'Remove',
    'cart.payment': 'Demo payment',
    'cart.confirm': 'Confirm demo order',
    'recap.title': 'Order confirmed',
    'recap.code': 'Your number',
    'recap.ready': 'Ready in about',
    'recap.payment': 'Payment',
    'recap.total': 'Total',
    'recap.allergens': 'Allergens',
    'recap.note': 'Note',
    'recap.contact': 'If anything is wrong, call the pizzeria.',
    'recap.sendSms': 'Send the summary by message',
    'recap.sendEmail': 'Send the summary by email',
    'recap.sent': 'Demo send: in the final version the summary really arrives.',
    'recap.newOrder': 'Place another order',
    'recap.demo': 'Demo order: no real payment was taken.',
    'payment.cash': 'Pay at the counter',
    'payment.apple_pay': 'Apple Pay · demo',
    'payment.google_pay': 'Google Pay · demo',
    'custom.title': 'Customise your pizza',
    'custom.included': 'Included ingredients',
    'custom.additions': 'Extras',
    'custom.removed': 'REMOVED',
    'custom.kept': 'INCLUDED',
    'custom.allergens': 'Allergens',
    'custom.none': 'none declared',
    'custom.note': 'Note for this pizza',
    'custom.notePlaceholder': 'E.g. nut allergy, coeliac, well done…',
    'custom.add': 'Add to cart',
    'custom.from': 'from',
    'captcha.missing': 'Please confirm you are not a robot (the box above), then send.',
    'custom.base': 'base price',
    'allergens.warning': 'If you have allergies or intolerances write it in the notes and contact the venue. Cross contamination may occur.'
  }
};

export function translate(key, locale = 'it') {
  return DICTIONARY[locale]?.[key] ?? DICTIONARY.it[key] ?? key;
}

export function translatePaymentMethod(methodId, locale = 'it') {
  if (!methodId) return '';
  const key = `payment.${methodId}`;
  const label = translate(key, locale);
  return label === key ? methodId : label;
}

export function translateProduct(names = {}, locale = 'it') {
  return names[locale] || names.it || '';
}
