...
let state=load(); let adminSection='service'; let customizing=null;
function load(){try{const saved=JSON.parse(localStorage.getItem('hm-state')||'{}');return {...defaults,...saved,menu:mergeMenuDefaults(saved.menu||[],defaults.menu)}}catch{return structuredClone(defaults)}}
...
function cart(){const total=state.cart.reduce((n,i)=>n+i.price,0);return `<button class="btn secondary" id="cart-close">Chiudi</button><h2>Il tuo ordine</h2>${state.cart.map((i,x)=>`<div class="card"><b>${i.name}</b><p>${i.removed?.length?`Senza: ${i.removed.join(', ')}<br>`:''}${i.additions?.filter(a=>a.quantity).map(a=>`${a.quantity}× ${a.name}`).join(', ')||''}</p><p>${i.note||'Nessuna nota'}</p><button data-remove="${x}">Rimuovi</button></div>`).join('')||'<p>Il carrello è vuoto.</p>'}<h3>Totale ${money(total)}</h3>${state.cart.length?`<div class="field"><label>Nome<input id="name"></label></div><div class="field"><label>Telefono<input id="phone" inputmode="tel"></label></div><div class="field"><span>Pagamento dimostrativo</span><div class="payment-grid">${DEMO_PAYMENT_METHODS.map((method,index)=>`<label class="payment-option"><input type="radio" name="payment" value="${method.id}" ${index===0?'checked':''}> <b>${method.label}</b></label>`).join('')}</div></div><button class="btn primary" id="checkout">Conferma ordine demo</button>`:''}`}
...
function orderCard(o){return `<article class="card order"><span class="pill">#${o.id} · ${o.source}</span><h3>${o.customer}</h3><p>${o.payment||'Pagamento non indicato'}</p>${o.items.map(itemDetails).join('')}</article>`}
function kitchen(){const active=state.orders.filter(o=>o.status==='preparing');return `<h1>Cucina</h1><p>${active.length} ordini in preparazione</p><div class="grid">${active.map(o=>{const seconds=Math.floor((o.readyAt-Date.now())/1000),timer=formatTimer(seconds);return `<article class="card order ${timer.late?'late':''}"><span class="pill">#${o.id} · ${o.source}</span><div class="timer">${timer.text}</div><p>Ordinato alle ${new Date(o.createdAt).toLocaleTimeString('it-IT',{hour:'2-digit',minute:'2-digit'})} · ${o.payment||'Pagamento non indicato'}</p>${o.items.map(itemDetails).join('')}<button class="btn primary ready" data-id="${o.id}">ORDINE PRONTO</button></article>`}).join('')||'<div class="card"><h2>Coda libera</h2><p>Nessuna comanda da preparare.</p></div>'}</div>`}
function bind(){
  ...
  document.querySelector('#checkout')?.addEventListener('click',()=>{
    ...
    if(!state.shift||!state.online)return toast('Il servizio online non è aperto.');
    ...
    const total=state.cart.reduce((n,i)=>n+i.price,0),eta=estimateMinutes(pizzasAhead(),state.capacity);
    state.orders.push({id:143+state.orders.length,source:'WEB',customer:document.querySelector('#name').value||'Cliente',phone,payment:payment.label,status:'preparing',shift:state.shift,createdAt:Date.now(),readyAt:Date.now()+eta*60000,total,fee:total*payment.feeRate,items:state.cart.map(i=>({...i,quantity:1}))});
  ...
  document.querySelectorAll('.shift').forEach(b=>b.onclick=()=>{const closing=state.shift===b.dataset.shift;state.shift=closing?null:b.dataset.shift;save();if(closing&&b.dataset.shift==='dinner')toast('Hai terminato la giornata lavorativa.');render()});
