...
import { buildCloseDialog, closeService, servicePanel, shiftLabel, startServiceTransition } from './views/service.js';
...
function bind(){
  ...
  document.querySelectorAll('.service-action').forEach(b=>b.onclick=()=>{
    ...
    const nextState=startServiceTransition(state,shift,Date.now(),action),closure=currentClosure(nextState.activeDay.date);
    if(closure.closed)return toast(closure.message);
    state=nextState;
    save();render();toast(action==='reopen'?'Servizio riaperto.':'Servizio aperto.');
  });
